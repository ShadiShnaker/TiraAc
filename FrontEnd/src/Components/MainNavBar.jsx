...
export default function MainNavBar(props){
    ...
    const logout = async () => {
      ...
      const getMemberPageLink = () => {
        if (props.isLoggedIn){
          return (
            <li className="nav-item"><Link to="/member" style={{textDecorationLine: "none"}}><a className="nav-link"  style={{color: isCurrentActivePage("/member")}}>Member Page</a></Link></li>
          );
        }
      }
          ...
          <div className="container"><a className="navbar-brand text-uppercase d-lg-none text-expanded" href="#" style={{color: 'rgb(47,123,211)'}}>Tira Academics</a><button data-bs-toggle="collapse" className="navbar-toggler" data-bs-target="#navbarResponsive"><span className="visually-hidden">Toggle navigation</span><span className="navbar-toggler-icon" /></button>
            <div className="collapse navbar-collapse" id="navbarResponsive">
              <ul className="navbar-nav mx-auto">
                <li className="nav-item"><Link to="/index" style={{textDecorationLine: "none"}}><a className="nav-link"  style={{color: isCurrentActivePage("/index")}}>Home</a></Link></li>
                <li className="nav-item"><Link to="/events" style={{textDecorationLine: "none"}}><a className="nav-link"  style={{color: isCurrentActivePage("/events")}}>Events</a></Link></li>
                ...
                {getMemberPageLink()}
                {props.isLoggedIn ? <li className="nav-item"><Link to="/login" onClick={() => logout()}  style={{textDecorationLine: "none"}}><a className="nav-link"  style={{color: isCurrentActivePage("/login")}}>log out</a></Link></li> : <li className="nav-item"><Link to="/login"  style={{textDecorationLine: "none"}}><a className="nav-link"  style={{color: isCurrentActivePage("/login")}}>member login</a></Link></li>}
              </ul>
            </div>
          </div>
        </nav>
