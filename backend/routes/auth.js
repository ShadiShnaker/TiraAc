...
router.post("/sendPasswordReset", verification.verifyToken, async (req, res) => {
    try{
        const user = await UserModel.findById(req._id);
...
router.post("/resetPassword", verification.verifyToken, async (req, res) => {
    try {
        await realmApp.emailPasswordAuth.resetPassword(req.body.password, req.body.token, req.body.tokenId);
        res.status(200).send();
    } catch (err) {
        res.status(500).send(err);
    }
